...
const { downloadGzipFileFromIMDb, dependencies } = require('./downloadGzipFileFromIMDb');

describe('downloadGzipFileFromIMDb', () => {
  let imdb;
  let mockWriteStream;
  let written;
  ...
  describe('when unzipped file already exists', () => {
    ...
    it('returns a promise', () => {
      const result = downloadGzipFileFromIMDb('testFile.gz', 'testPath/20180826', jest.fn(), jest.fn());
    ...
    it('does not call onDownloadProgress callback', () => {
      ...
      downloadGzipFileFromIMDb('testFile.gz', 'testPath/20180826', onDownloadProgress, jest.fn());
      expect(onDownloadProgress).not.toHaveBeenCalled();
    });

    it('calls onFinish callback', () => {
      const onFinish = jest.fn();
      downloadGzipFileFromIMDb('testFile.gz', 'testPath/20180826', jest.fn(), onFinish);
  ...
  describe('when unzipped file does not exist', () => {
    ...
    describe('when download is successful', () => {
      ...
      it('calls onDownloadProgress callback', () => {
        const onDownloadProgress = jest.fn();
        return downloadGzipFileFromIMDb('testFile.gz', 'testPath/20180826', onDownloadProgress, jest.fn())
          .then(() => expect(onDownloadProgress).toHaveBeenCalled());
      });

      it('calls onFinish callback', () => {
        const onFinish = jest.fn();
        return downloadGzipFileFromIMDb('testFile.gz', 'testPath/20180826', jest.fn(), onFinish)
